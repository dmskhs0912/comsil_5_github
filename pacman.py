...
class PacmanRules:
  ...
  def getLegalActions( state ): # 현재 gamestate를 인자로 받음

    possibleActions = Actions.getPossibleActions( state.getPacmanState().configuration, state.data.layout.walls )
    if Directions.STOP in possibleActions: # STOP이 있는 경우 제거해야함.
  ...
  def applyAction( state, action ):
    ...
    if action not in legal:
      raise Exception("Illegal action " + str(action)) # action이 legal하지 않은 경우 예외처리
...
class GhostRules:
  ...
  def getLegalActions( state, ghostIndex ):
    ...
    if Directions.STOP in possibleActions: # STOP 제거 
      possibleActions.remove( Directions.STOP )
